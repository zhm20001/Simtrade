# SimTrade 模拟交易系统 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a single-file CLI simulated stock trading system that LLMs can call via shell to trade A-shares.

**Architecture:** Single `simtrade.py` file (~500-600 lines) containing embedded Ashare market data functions, a trading engine with commission/slippage simulation, JSON-based portfolio persistence, and argparse CLI. All output is JSON for LLM parsing.

**Tech Stack:** Python 3, requests, pandas (already installed)

**Spec:** `docs/superpowers/specs/2026-06-09-simtrade-design.md`
...
| File | Responsibility |
|------|---------------|
| `simtrade.py` | Entire system: config, market data, storage, trading engine, CLI |
| `config.json` | Optional user config (created on `init` with defaults) |
| `data/portfolio.json` | Account state (auto-created) |
| `data/trades.csv` | Trade log (auto-created) |
| `data/stock_names.json` | Code→name cache (auto-created) |

## Build Order

Tasks build on each other. Each task produces a runnable, testable increment.

---

### Task 1: Scaffolding — config, constants, and error output

**Files:**
- Create: `simtrade.py`

This task creates the file with all imports, default config, config loading, data directory init, and the JSON error output helper. Not yet runnable as a full CLI, but establishes the foundation.

- [ ] **Step 1: Create simtrade.py with imports, config defaults, config loader, and error helper**
...
Run: `cd ~/simtrade && python -c "import simtrade; print('OK')"`
Expected: `OK`

- [ ] **Step 3: Commit**

```bash
cd ~/simtrade && git add simtrade.py && git commit -m "feat: scaffolding with config, constants, and error output"
```

---

### Task 2: Ashare market data functions (embedded)

**Files:**
- Modify: `simtrade.py`

Embed the Ashare get_price functions directly into simtrade.py, plus a `get_realtime_price()` helper and stock name resolution.

- [ ] **Step 1: Add Ashare functions and realtime price helper after the config section**

Append after `json_error()` function, before any CLI code:
...
- [ ] **Step 2: Verify syntax and that get_price works**

Run: `cd ~/simtrade && python -c "from simtrade import get_price; print(get_price('sh600519', frequency='1d', count=2))"`
Expected: DataFrame with 2 rows of daily data for 贵州茅台

- [ ] **Step 3: Commit**

```bash
cd ~/simtrade && git add simtrade.py && git commit -m "feat: embed Ashare market data functions with name resolution"
```

---

### Task 3: Data persistence — portfolio and trades

**Files:**
- Modify: `simtrade.py`

Add portfolio load/save, trades append/read functions.

- [ ] **Step 1: Add storage functions after the stock name section**
...
Run: `cd ~/simtrade && python -c "from simtrade import load_portfolio, save_portfolio, append_trade, load_trades; print('OK')"`
Expected: `OK`

- [ ] **Step 3: Commit**

```bash
cd ~/simtrade && git add simtrade.py && git commit -m "feat: add portfolio and trades persistence functions"
```

---

### Task 4: Trading engine — time check, commission, buy, sell

**Files:**
- Modify: `simtrade.py`

Add trading hours check, commission calculation, and core buy/sell execution.

- [ ] **Step 1: Add trading engine functions after the storage section**
...
Run: `cd ~/simtrade && python -c "from simtrade import execute_buy, execute_sell, is_trading_hours; print('OK')"`
Expected: `OK`

- [ ] **Step 3: Commit**

```bash
cd ~/simtrade && git add simtrade.py && git commit -m "feat: add trading engine with buy/sell execution"
```

---

### Task 5: CLI commands — init, status, quote, quotes, history, pnl, reset

**Files:**
- Modify: `simtrade.py`

Add all CLI command handler functions and the argparse main entry point.

- [ ] **Step 1: Add CLI command handlers and main entry point after the trading engine**
...
Run: `cd ~/simtrade && python simtrade.py --help`
Expected: argparse help listing all commands

Run: `cd ~/simtrade && python simtrade.py init --cash 1000000`
Expected: JSON with `{"status": "ok", "cash": 1000000.0, ...}`

- [ ] **Step 3: Commit**

```bash
cd ~/simtrade && git add simtrade.py && git commit -m "feat: complete CLI with all commands (init/status/quote/trade/history/pnl/reset)"
```
...
Run a full workflow to verify everything works together.

- [ ] **Step 1: Initialize account**

Run: `cd ~/simtrade && python simtrade.py init --cash 1000000`
Expected: JSON with cash=1000000

- [ ] **Step 2: Check status**

Run: `cd ~/simtrade && python simtrade.py status`
Expected: JSON with cash=1000000, positions=[], total_assets=1000000

- [ ] **Step 3: Get a quote**

Run: `cd ~/simtrade && python simtrade.py quote sh600519`
Expected: JSON with code, name, price > 0

- [ ] **Step 4: Buy with --force (bypass trading hours)**

Run: `cd ~/simtrade && python simtrade.py buy sh600519 1500.00 100 --force --reason "test buy"`
Expected: JSON with status=ok, qty=100

- [ ] **Step 5: Check status shows position**

Run: `cd ~/simtrade && python simtrade.py status`
Expected: JSON with positions containing sh600519, cash reduced

- [ ] **Step 6: Sell with --force**

Run: `cd ~/simtrade && python simtrade.py sell sh600519 1550.00 100 --force --reason "test sell"`
Expected: JSON with status=ok, cash increased

- [ ] **Step 7: Check history**

Run: `cd ~/simtrade && python simtrade.py history`
Expected: JSON with 2 trades (1 buy, 1 sell)

- [ ] **Step 8: Check P&L**

Run: `cd ~/simtrade && python simtrade.py pnl`
Expected: JSON with realized_pnl > 0

- [ ] **Step 9: Reset and verify**

Run: `cd ~/simtrade && python simtrade.py reset && python simtrade.py status`
Expected: cash=1000000, positions=[]

- [ ] **Step 10: Commit verified state**

```bash
cd ~/simtrade && git add -A && git commit -m "test: end-to-end smoke test passed, data files created"
```

---

### Task 7: .gitignore and final cleanup

**Files:**
- Create: `.gitignore`

Exclude runtime data files from version control.
...
- [ ] **Step 2: Remove data files from git tracking if tracked**

```bash
cd ~/simtrade && git rm -r --cached data/ 2>/dev/null; git add .gitignore && git commit -m "chore: add .gitignore, exclude runtime data"
```

- [ ] **Step 3: Final verification**

Run: `cd ~/simtrade && python simtrade.py --help`
Expected: Clean help output with all 11 commands listed
